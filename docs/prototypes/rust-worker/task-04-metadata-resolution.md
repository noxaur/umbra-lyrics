...
- Rust native tests, clippy, Wasm check, and `worker-build` could not run in
  this Windows workspace: MSVC `link.exe`/Windows SDK libraries are absent,
  and the standalone Rust install has no `rustup`-managed Wasm target.
- Focused frontend resolver/gateway verification passed: 7 passed, 12 skipped.
  Full frontend suite: 875 passed, 29 skipped, 1 unrelated existing UI test
  timed out (`tests/pages/playlist-cache-sync.test.tsx`).
