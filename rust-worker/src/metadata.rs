...
async fn fetch_oembed(
    ...
    for host in ["music.youtube.com", "www.youtube.com"] {
        ...
        match fetch_json(url, MetadataSource::Oembed, config, &[], "YouTube oEmbed").await {
            ...
            Ok(response) => {
                candidates.extend(response.recordings.into_iter().filter_map(|recording| {
                    ...
                    Some(MetadataCandidate {
                        ...
                        source_id: Some(recording.id.clone()),
                        stable_ids: StableIds {
                            youtube_video_id: Some(input.video_id.clone()),
...
async fn fetch_deezer(
    ...
    for seed in seeds {
        ...
        url.query_pairs_mut()
            .append_pair("q", &query)
            .append_pair("limit", "5");
        match fetch_json::<DeezerResponse>(url, MetadataSource::Deezer, config, &[], "Deezer").await
        {
            Ok(response) => candidates.extend(response.data.into_iter().filter_map(|track| {
                let title = track.title.trim().to_owned();
                if track.id == 0 || title.is_empty() {
                    return None;
                }
...
async fn fetch_json<T: DeserializeOwned>(
    ...
    let mut response = match select(fetch, timeout).await {
        Either::Left((result, _)) => result.map_err(|error| SourceFailure {
            source,
            code: "source_unavailable",
