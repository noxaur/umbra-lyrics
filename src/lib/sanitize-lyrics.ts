...
const JUNK_LINE =
  /(?:document\.write|cf_async|clickfuse|adunit_id|function\s*\(|^\s*var\s+\w+\s*=|^\s*\}\)\(\)|\/\* Lyrics\.net|srv\.clickfuse|Math\.random|getElementsByTagName|<script|^\s*\{?\s*$|^\s*\};?\s*$|^PDF$|^Playlist$)/i
