...
describe("lyrics-providers index", () => {
  ...
  it("waits for lrclib before settling on scraper fallback", async () => {
    const completed: LyricsProviderId[] = []
  ...
  it("waits for lrclib before settling when scrapers return first", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string) => {
        if (url.includes("/api/lyrics/lrclib") || url.includes("/search")) {
        ...
        if (url.includes("/api/lyrics/ovh/")) {
          return new Response(
            JSON.stringify({ lyrics: "Scraper junk line one\nTwo\nThree\nFour" }),
            { status: 200 },
          )
        }
        return new Response("[]", { status: 200 })
  ...
  it("accepts plain fallback after lrclib returns empty", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string) => {
        if (url.includes("/api/lyrics/lrclib")) {
